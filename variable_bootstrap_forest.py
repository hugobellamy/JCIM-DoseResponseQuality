...
class vbRandomForest:
    ...
    def fit(self, X, y, probs=None, sample_weight=None):
        if probs is None and sample_weight is not None:
            probs = sample_weight

        if sample_weight is None and probs is not None:
            sample_weight = probs

        if probs is None and sample_weight is None:
            probs = np.ones(len(X))
